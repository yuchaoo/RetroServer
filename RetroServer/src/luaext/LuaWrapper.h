...
static int Lua_Error(lua_State* L)
{
    ...
    LogUtils::LogDebug("/***********************trace back ended****************************/");
    return 0;
}
...
static int Lua_Loader(lua_State* L)
{
    ...
    if (buffer.get() == NULL)
    {
        LogUtils::LogDebug("load the lua file %s failed!!!",filepath.c_str());
        return 0;
    }
    LogUtils::LogDebug("load the lua file : %s", filepath.c_str());
	int ret = luaL_loadbuffer(L, (char*)buffer.get(), size, ("@" + filepath).c_str());
	if (ret != 0)
	{
		const char* str = lua_tostring(L, -1);
		LogUtils::LogDebug("load script %s failed!!!, msg:%s", path.c_str(), str);
		lua_pop(L, 1);
		return 0;
	}
	return 1;
...
static int Lua_Class(lua_State* L)
{
	...
	if (super)
	{
		...
		if (lua_isnil(L, -1))
		{
			LogUtils::LogDebug("can not create class %s , reason : can not find the super class %s", dir, super);
			return 0;
		}
		lua_getfield(L, -2, dir);
		if (lua_isnil(L, -1))
		{
			LogUtils::LogDebug("%s have be a class", dir);
			return 2;
		}
		lua_pop(L, 1);
...
static int Lua_SetMetatable(lua_State* L, int index, const char* super)
{
    ...
    {
        LogUtils::LogDebug("can not find the super : %s",super);
        lua_pop(L, 2);
    }
    return 0;
}
...
static int Lua_CreateModule(lua_State* L, const char* nname, luaL_Reg* fn, const char* super)
{
    ...
    LogUtils::LogDebug("the module:%s has exist",nname);
	lua_pop(L, 2);
    return 0;
}
...
{
    if (lua_rawgeti(L, LUA_REGISTRYINDEX, ptr->_luaID) != LUA_TNIL)
    {
        return 1;
    }
...
static bool Lua_DoFile(lua_State* L, const char* filename,lua_CFunction fn)
{
    ...
    {
        const char* err = lua_tostring(L, -1);
        LogUtils::LogDebug("load file %s failed, err:%s",filename,err);
        lua_pop(L, fn ? 2 : 1);
        return false;
    }
...
static int Lua_TopField(lua_State* L, const std::string& key)
{
    if (lua_istable(L, -1))
    {
